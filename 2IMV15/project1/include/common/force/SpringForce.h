...
private:
    Particle *const _p1;    // particle 1
    Particle *const _p2;    // particle 2
    double const _dist;     // rest length
    double const _ks, _kd;  // spring strength constants
public:
    ...
    std::map<int, std::map<int, float>> dx() override {
        ...
        Vec2f length = _p1->pos() - _p2->pos();
        Vec2f length_derivate = _p1->v() - _p2->v(); // l'=velocity p1-velocity p2
        Eigen::Vector2f xij = Eigen::Vector2f(length[0], length[1]);
        Eigen::Vector2f vij = Eigen::Vector2f(length_derivate[0], length_derivate[1]);
