...
/************************************************************************
  ...
  $Id: vec2._h 427 2004-09-27 04:45:31Z garland $
...
#if _MSC_VER>=1200
// Normally, we use the <class T, class _n> construct below to allow the scalar
// argument to be different than the template type.  This, for example, allows
// the user to write things like _v/2.  Unfortunately, Microsoft VC6.0 (aka
...
//
  ...
  template<class T> inline TVec2<T> operator*(T s, const TVec2<T> &_v)
	{ return TVec2<T>(_v[0]*s, _v[1]*s); }
  template<class T> inline TVec2<T> operator*(const TVec2<T> &_v, T s)
	{ return s*_v; }
